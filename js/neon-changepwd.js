...
;(function($, window, undefined)
{
	...
	$(document).ready(function()
	{
		...
		if(is_lockscreen)
		{
			$(".form-login-error").slideUp();
			$(".form-forgotpassword-success").slideUp();
					...
					setTimeout(function()
					{
						...
						neonLogin.setPercentage(random_pct, function()
						{
							...
							// After 0.77s second we will execute the next phase
								neonLogin.setPercentage(98, function()
								{
								$.ajax({
									url: baseurl + 'data/changepwd2.php',
									method: 'POST',
									dataType: 'json',
									data: {
										password: $("input#password").val(),
										confpassword: $("input#confpassword").val(),
										username : $("input#AjaDecade").val(),
										},
										error: function()
										{
											alert("An error occoured!");
										...
										success: function(response)
										{
											var pass_status = response.pass_status;
											neonLogin.setPercentage(100);
											setTimeout(function(){
											$("#form_lockscreen").slideUp();
											},650);
											setTimeout(function(){
		...
		$.extend(neonLogin, {
			...
			resetProgressBar: function(display_errors)
			{
				...
				setTimeout(function()
				{
					...
					if(display_errors)
					{
						var $errors_container = $("#error-login");
