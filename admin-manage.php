...
include_once('include/lang/' . $myLang . '.php');
...
mysqli_query($con, "INSERT INTO `package` (`color`, `name`, `price`, `length`, `generator`, `generations`, `haspriv`, `privgenerations`) VALUES ('$colorpackage', '$name', '$price', '$length', '$generator', '$accounts', '$haspriv', '$privgenerations')") or die(mysqli_error($con));
	...
	if(isset($_POST['website'])){
		...
		$data = array("website" => $_POST['website'], "link" => $_SERVER['HTTP_HOST']);
		$string = http_build_query($data);
		
		$ch = curl_init("https://h2mdev.net/check.php");
...
<head>
	...
	<title><?php echo $website ?> | <?php echo $lang['manage']; ?></title>
...
<?php include'include/header.php'; ?>
		<hr />
					...
					<ol class="breadcrumb bc-3" >
								<li>
						<a href="index"><i class="fa-home"></i><?php echo $lang['home']; ?></a>
					</li>
						<li>
							...
							<li class="active">
									...
									<strong><?php echo $lang['manage']; ?></strong>
							</li>
							</ol>
...
<div class="col-lg-3">
				...
				<div class="panel panel-info" data-collapsed="0">
					...
					<div class="panel-heading" >
						<div class="panel-title" style="font-color:white;"><?php echo $lang['manage_premium_gen']; ?></div>
...
echo '</textarea>
<br>
<button type="submit" class="btn btn-info btn-large btn-block">' .$lang['update_accounts']. '</button>
</form>
    </center>
      </div>
    </div>
...
<button type="submit" class="btn btn-blue btn-large btn-block"><?php echo $lang['add_accounts']; ?></button>
</form>
					</div>
...
<div class="col-lg-3">
				...
				<div class="panel panel-info" data-collapsed="0">
					...
					<!-- panel head -->
					<div class="panel-heading" >
						<div class="panel-title" style="font-color:white;"><?php echo $lang['manage_private_generator']; ?></div>
...
<button style="margin-bottom: 5px;" class="btn btn-red popover-secondary" data-toggle="popover" data-trigger="hover" data-placement="bottom" data-content="'. $lang['add_an_acc'] .'." data-original-title="'. $lang['how_to_enable_it'] .'?">' . $lang['pgen_is_disabled'] . '.</button>
</center>
';
}else{
while($row = mysqli_fetch_assoc($accountsquery)){
...
<div class="col-lg-3">
				...
				<div class="panel panel-info" data-collapsed="0">
					...
					<!-- panel head -->
					<div class="panel-heading" >
						<div class="panel-title" style="font-color:white;"><?php echo $lang['manage_packages']; ?></div>
...
<div class="modal fade" id="sample-modal-dialog-'.$idpackage.'">
		<div class="modal-dialog">
            ...
            <input type="text" name="edit-namepackage" class="form-control"  value="'.$accountname.'" required/></br>
					<label>' .$lang['price']. ' (<span class="badge badge-info" style="color:white; text-align: right;">'. $value .'</span>) :</label></br>
            <input type="text" name="edit-pricepackage" class="form-control"  value="'. $price .'" required/></br>
								<label>' .$lang['generations']. ' (<span class="badge badge-info" style="color:white; text-align: right;">'. $lang['per_day'] .'</span>):</label></br>
			...
			<label>' .$lang['private_generations']. ' (<span class="badge badge-info" style="color:white; text-align: right;">'. $lang['per_day'] .'</span>):</label></br>
            <input type="text" name="edit-privgen" class="form-control" placeholder="' .$lang['leave_empty_unlimited']. '" value="'. $privgenerationss .'" onKeydown="chiffres(this)" /></br>
            ...
            <input type="text" name="idpackage" class="form-control"  value="'. $idpackage.'" readonly/></br></div>
				...
				<div class="modal-footer">
					...
					<button type="button submit" class="btn btn-primary">' .$lang['save_changes']. '</button>
					</form>
				</div>
			</div>
		</div>
  ...
  <hr />
  <center><i>
  <p><?php echo $lang['create_package']; ?>.</p>
					</i>
							</center>
    					<form action="admin-manage" method="POST">
<input type="text" name="name" class="form-control" placeholder="<?php echo $lang['name']; ?>" style="margin-bottom: 5px;" required>
<input type="text" name="price" class="form-control" placeholder="<?php echo $lang['price']; ?>" style="margin-bottom: 5px;" required>
<input type="text" name="accounts" onKeydown="chiffres(this)" class="form-control" placeholder="<?php echo $lang['max_acc_per_day']; ?>(<?php echo $lang['leave_empty_unlimited']; ?>)" style="margin-bottom: 5px;">
<select name="generator" class="form-control" style="margin-bottom: 5px;" required>
<option value="all"><?php echo $lang['all_generators']; ?></option>
<?php 
$accountsquery = mysqli_query($con, "SELECT * FROM `generators`") or die(mysqli_error($con));
while($row = mysqli_fetch_assoc($accountsquery)){
...
<option value=""><?php echo $lang['color']; ?>:</option>
<option value="orange"><?php echo $lang['orange']; ?></option>
<option value="red"><?php echo $lang['red']; ?></option>
<option value="normal"><?php echo $lang['purple']; ?></option>
</select>
<select name="length" class="form-control" style="margin-bottom: 5px;" required>
<option value="Lifetime"><?php echo $lang['lifetime']; ?></option>
<option value="1 Day">1 <?php echo $lang['day']; ?></option>
<option value="3 Days">3 <?php echo $lang['days']; ?></option>
<option value="1 Week">1 <?php echo $lang['week']; ?></option>
<option value="1 Month">1 <?php echo $lang['month']; ?></option>
<option value="2 Months">2 <?php echo $lang['months']; ?></option>
<option value="3 Months">3 <?php echo $lang['months']; ?></option>
<option value="4 Months">4 <?php echo $lang['months']; ?></option>
<option value="5 Months">5 <?php echo $lang['months']; ?></option>
<option value="6 Months">6 <?php echo $lang['months']; ?></option>
<option value="7 Months">7 <?php echo $lang['months']; ?></option>
<option value="8 Months">8 <?php echo $lang['months']; ?></option>
<option value="9 Months">9 <?php echo $lang['months']; ?></option>
<option value="10 Months">10 <?php echo $lang['months']; ?></option>
<option value="11 Months">11 <?php echo $lang['months']; ?></option>
<option value="12 Months">12 <?php echo $lang['months']; ?></option>
</select>



<select name="haspriv" id="colorselector" class="form-control" style="margin-bottom: 5px;" required>
<option value=""><?php echo $lang['has_private_generator']; ?>?</option>
...
<input type="text" name="privgenerations" class="form-control" placeholder="<?php echo $lang['how_man_private_generations']; ?>? (<?php echo $lang['empty_unlimited']; ?>)" style="margin-bottom: 5px;">
</div>




<button type="submit" class="btn btn-blue btn-large btn-block"><?php echo $lang['add_package']; ?></button>
</form>
  </div>
  </div>
				</div>
				<div class="col-lg-3">
					...
					<!-- panel head -->
					<div class="panel-heading" >
						<div class="panel-title" style="font-color:white;"><?php echo $lang['manage_settings']; ?></div>
            ...
            <button class="btn btn-red btn-large btn-block"><?php echo $lang['update_settings']; ?></button>
            </form>
            </div>
                    </div>
  </div>
			...
			<div class="col-md-12">
				<div class="panel panel-success" data-collapsed="0">
					...
					<!-- panel head -->
					<div class="panel-heading" >
						<div class="panel-title" style="font-color:white;"><?php echo $lang['manage_news']; ?></div>
...
<textarea type="text" name="message" class="form-control" placeholder="<?php echo $lang['write_your_news_here']; ?>..." rows="3" style="margin-bottom: 5px;" required></textarea>
<label><?php echo $lang['writer']; ?>:</label></br>
...
<button type="submit" class="btn btn-success btn-large btn-block"><?php echo $lang['add_news']; ?></button>
</form>
<hr />
</div>
  <div class="col-md-6">
...
<div class="modal fade" id="sample-modal-dialog-'.$idnew.'">
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
					<h4 class="modal-title">' .$lang['configure_news']. ' n°'.$idnew . '</h4>
				</div>
